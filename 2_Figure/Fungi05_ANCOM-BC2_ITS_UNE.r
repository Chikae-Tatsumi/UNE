...
seqtab.nochim <- read.table("~/R/Analysis/2_UNE/ITS/seqtabnochim.txt",header=T)
seqtab.nochim <- as.matrix(seqtab.nochim)
taxa <- read.table("~/R/Analysis/2_UNE/ITS/taxonomy.txt",header=T)
taxa <- as.matrix(taxa)
...
ps.ancom = subset_taxa(ps.ancom,(Kingdom  == "k__Fungi" ))
