...
ASV.table <- read.table(file="ASV_table.txt",header=T)
...
max <- max(na.omit(data$Dissimilarity))
min <- min(na.omit(data$Dissimilarity))
data$Similarity <- 1-(data$Dissimilarity-min)/(max-min)
write.csv(data, "ITS_Stability_withAveragedMetadata.csv")
...
min <- min(na.omit(data$Dissimilarity))
data$Similarity <- 1-(data$Dissimilarity-min)/(max-min)
write.csv(data, "16S_Stability_withAveragedMetadata.csv")
