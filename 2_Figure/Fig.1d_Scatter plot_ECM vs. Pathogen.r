...
geom_smooth(method="lm", aes(x=ECM, y=Pathogenic_capacity, group=Type),color="black")+
scale_shape_manual(values = c(16,2))+ 
theme_classic()+
theme(text=element_text(size=14,color="black"),axis.text=element_text(size=14,color="black"))+
labs (x="Soil ECM fungi (%)",y="Soil pathogenic fungi (%)",color="", shape="")  

ggsave("ECM.vs.Pathogen_UNE.png",width = 5, height = 4)
