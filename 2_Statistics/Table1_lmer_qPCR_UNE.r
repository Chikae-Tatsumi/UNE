...
data <- na.omit (data)
model <- lmer(scale(log10(data[,ncol(data)]+0.1))~ scale(-DFB)*scale(-DFE)+(1|Site),data=data) # To log-transform and to use 0 copy/g data
anova <- anova(model)
...
# Save
write.csv(Results.asterisk, "lmer.qPCR.csv") 
