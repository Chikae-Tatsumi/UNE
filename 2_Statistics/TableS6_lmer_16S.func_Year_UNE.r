...
# Save
write.csv(Results.asterisk, "lmer.16S.Urban.Year.csv") 
...
# Save
write.csv(Results.asterisk, "lmer.16S.Rural.Year.csv") 
...
# Save
write.csv(Results.asterisk, "lmer.16S.Edge.Year.csv") #Change



###### For Interior ######
bind <- cbind (objective,DESIGN)
subset <- subset(bind, bind$Edge=="Interior") # Change
subset.ASV <- subset[,1:(ncol(subset)-ncol(DESIGN))]
DESIGN.subset <- subset[,(ncol(subset)-ncol(DESIGN)+1):ncol(subset)]
...
# Save
write.csv(Results.asterisk, "lmer.16S.Interior.Year.csv") #Change
